...
export default function Home() {
    ...
    const validateForm = () => {
        ...
        if (!formDetails.user_name.trim()) {
            isError = true,
                errorObj.user_name = 'Full name is required!'
        }
        ...
        if (!formDetails.phone_number) {
            isError = true,
                errorObj.phone_number = 'Phone number is required!'
        }

        if (!formDetails.terms_and_condition) {
            isError = true,
                errorObj.terms_and_condition = 'Kindly accpect our terms and condition before submitting form!'
        }

        if (!formDetails.user_email.trim()) {
            isError = true;
            errorObj.user_email = 'Email Id is required!';
        } else if (!emailPattern.test(formDetails?.user_email)) { // Validate only if email is provided
            isError = true;
    ...
    const handleFormChange = (e) => {
        ...
        if (name === 'phone_number') {
            ...
            if (value.length > 10) {
                toast.error('Maximum limit of phone number reached!');
                return;
            }
        }

        setFormDetails((preve) => ({
            ...preve,
            [name]: value
        }))
    ...
    const handleFormSubmit = async (e) => {
        e.preventDefault()
        if (validateForm()) {
            await sendEmail(tempId, formDetails)
            setFormDetails({
                user_name: "", user_email: '', phone_number: "", subject: '', message: "", terms_and_condition: ""
            })
        } else {
            toast.error('Form has validation error please make sure every field is filled')
        }
    }
        ...
        <>
            <section className="relative z-10 h-screen text-white overflow-hidden">
                <div className="absolute inset-0">
                    <Img src={'https://res.cloudinary.com/dmh7fwdzf/image/upload/v1725907997/Home_dztofa.png'} alt="Background Image" className="object-cover object-center w-full h-full" />
            ...
            <section className='bg-[#d9d9d9] relative isolate overflow-hidden'>
                <div className="container mx-auto flex px-5 py-12 md:flex-row md:gap-12 justify-center flex-col items-center">
                    <div className="lg:max-w-lg lg:w-full md:w-1/2 w-full">
                        <img className="w-full h-full object-cover object-center rounded-xl" alt="whychooseus" src={"https://res.cloudinary.com/dmh7fwdzf/image/upload/v1725907911/whychooseus_fkbqwx.png"} />
                    </div>
                    <div
            ...
            {/* Testimonials */}
            <section className=' relative isolate overflow-hidden  bg-[#d9d9d9] px-3 py-4 lg:px-8 lg:py-6'>
                            ...
                            <div className="px-4 py-4 md:py-6 md:px-6">
                                <div className="flex justify-start items-center">
                                    <div className='w-12 rounded-full bg-gray-400 h-8 flex justify-center items-center'>
                                        <ImageTag src={'https://res.cloudinary.com/dmh7fwdzf/image/upload/v1725907812/profile_te6siy.png'} className={'w-12 h-12 object-fill'} alt={'profil'} />
                                    </div>
            ...
            <section className='relative isolate overflow-hidden min-h-dvh bg-[#d9d9d9]'>
                <Img src={'https://res.cloudinary.com/dmh7fwdzf/image/upload/v1725907993/CTAedit_yhxora.png'} alt="Background Image" className="absolute inset-0 -z-0 object-cover object-center w-full h-full" />
                <div className="absolute inset-0 bg-black opacity-20"></div>
                <div className='mx-auto max-w-sm lg:max-w-screen-lg'>
                    <div className='absolute z-20 inset-0 max-w-sm md:max-w-md top-0 md:top-1/4'>
                        <div className="px-3 py-3 md:py-5 md:px-5 text-center md:text-start md:max-w-md">
                            <h1 className='text-white font-Poppins  leading-7 hidden md:inline-block md:leading-relaxed tracking-wide font-extrabold text-2xl lg:text-4xl'>Ready to Transform Your Logistics?</h1>
                        </div>
                        <div className="max-w-sm px-3 md:px-5">
                            <p className='text-xs md:inline-block hidden md:text-sm text-white font-normal text-center md:text-start leading-snug tracking-wider font-Poppins'>Join us in revolutionizing the logistics industry. Discover how our services can benefit your business.</p>
                        </div>
                    </div>

                    <div className="absolute flex justify-center items-center h-auto inset-0 top-0 z-20 rounded-md sm:left-1/2 drop-shadow-lg backdrop-blur-sm bg-black/20">
                        <div className="max-w-full lg:max-w-md mx-auto">
                            <div className="px-3 py-3 md:px-4 md:py-4">
                                ...
                                <form onSubmit={handleFormSubmit} onFocus={handleFormFocus} onChange={handleFormChange} className='space-y-1 md:space-y-2'>
                                    <div className="relative">
                                        <label htmlFor="user_name" className="text-white tracking-wider text-xs font-Poppins font-normal mb-1">Full name</label>
                                        <input type="text" id="user_name" name='user_name' placeholder='Full name' className={`w-full ${error.user_name && 'ring-1 ring-red-600 placeholder:text-red-500'} focus:outline-none font-Poppins text-xs rounded-md border py-1.5 px-1.5 bg-white text-black border-none placeholder:text-[8px] placeholder:tracking-wide`} />
                                    ...
                                    <div className="grid md:grid-cols-2 grid-cols-1 gap-y-2 sm:gap-x-2">
                                        <div className='block relative'>
                                            ...
                                            <input type="email" id='user_email' placeholder='Email' name='user_email' className={`w-full ${error.user_email && 'ring-1 ring-red-600 placeholder:text-red-500'} focus:outline-none font-Poppins text-xs rounded-md border py-1.5 px-1.5 bg-white text-black border-none placeholder:text-[8px] placeholder:tracking-wide`} />
                                            <span className="absolute inset-y-0 top-6 end-0 grid place-content-center px-2 pointer-events-none">
                                        ...
                                        <div className='block relative'>
                                            <label htmlFor='phone_number' className='text-white tracking-wider text-xs font-Poppins mb-1 font-normal'>Phone number</label>
                                            <input type="number" min={1} max={10} id='phone_number' placeholder='Phone number' name='phone_number' className={`w-full ${error.phone_number && 'ring-1 ring-red-600 placeholder:text-red-500'} focus:outline-none font-Poppins text-xs rounded-md border py-1.5 px-1.5 bg-white text-black border-none placeholder:text-[8px] placeholder:tracking-wide`} />
                                            <span className='absolute inset-y-0  top-6 end-0 grid place-content-center px-2 pointer-events-none'>
                                    ...
                                    <div className="block relative">
                                        <div className="">
                                            ...
                                            <input type="text" id="subject" name='subject' placeholder='Subject' className={`w-full ${error.subject && 'ring-1 ring-red-600 placeholder:text-red-500'} focus:outline-none font-Poppins text-xs rounded-md border py-1.5 px-1.5 bg-white text-black border-none placeholder:text-[8px] placeholder:tracking-wide`} />
                                            <span className='absolute inset-y-0 top-6 end-0 grid place-content-center px-2 pointer-events-none'>
                                    ...
                                    <div className="">
                                        <div className="flex items-center">
                                            <input type="radio" name="terms_and_condition" id="terms_and_condition" className="h-2.5 w-2.5" />
                                            <label htmlFor="terms_and_condition" className="pl-3 lg:text-[8px] text-[6px] -tracking-tight leading-3 font-medium text-white font-Poppins">
                                        ...
                                        {error.terms_and_condition && (
                                            <span className='text-xxs font-medium font-Poppins leading-6 tracking-wide text-red-600'>{error.terms_and_condition}</span>
                                        )}
                                    </div>
